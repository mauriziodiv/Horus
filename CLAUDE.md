...
- **`roughness` is a lobe probability, not a gloss width** — `rayPath()` draws `rand() < roughness` (`render.cpp:404`) and takes *either* a cosine-weighted diffuse bounce *or* a perfect mirror reflection. There is no glossy lobe, so a low roughness gives a sharp mirror, not a soft sheen. The old blend `reflectedDir * (1 - roughness) + diffuseScatter * roughness` is commented out at `render.cpp:620`.
- **The mirror lobe is tinted by `diffuseColor`** (`render.cpp:634`) — physically wrong for a dielectric; it's the known specular-tint bug, not an accident of the scene.
